...
window.customElements.define("stock-countdown", StockCountdown);
  ...
  */
  ...
  var DiscountShowcase = class {
    ...
    async init() {
      // Load prefixes and cart in parallel for faster init
      const [prefixes, cart] = await Promise.all([
      ...
      document.documentElement.addEventListener("product-facet:rerender", () => {
        requestAnimationFrame(() => {
          this.updateAllShowcases();
        });
      });
      ...
      try {
        if (window.themeVariables?.discountPrefixes) {
          ...
          Object.keys(prefixesCopy).forEach(prefix => {
            ...
            if (prefixWithoutUnderscore !== prefix && !this.discountPrefixes[prefixWithoutUnderscore]) {
    ...
    async updateAllShowcases() {
      const showcases = document.querySelectorAll('[data-discount-showcase]');
      ...
      try {
        ...
        // Hide all showcases on error to prevent showing incorrect info
        document.querySelectorAll('[data-discount-showcase]').forEach(el => {
          this.hideShowcase(el);
        });
      }
    }

    getDiscountCodeFromCart(cart) {
      if (!cart || !cart.discount_codes || cart.discount_codes.length === 0) {
        return null;
      }
      return cart.discount_codes[0].code;
    }


    async updateShowcase(showcaseElement, discountCode, cart) {
      const productPrice = parseFloat(showcaseElement.getAttribute('data-product-price')) || 0;
      const fallbackCode = showcaseElement.getAttribute('data-fallback-code') || '';
      const fallbackPercentage = parseFloat(showcaseElement.getAttribute('data-fallback-percentage')) || 0;
      const showSecondBadge = showcaseElement.getAttribute('data-show-second-badge') === 'true';
      ...
      if (!activeCode || discountPercentage === 0) {
        this.hideShowcase(showcaseElement);
        return;
      }

      const discountedPrice = productPrice * (1 - discountPercentage / 100);
      const savingsPercentage = Math.round(discountPercentage);

      const formattedPrice = this.formatMoney(discountedPrice);

      const priceElement = showcaseElement.querySelector('[data-discount-price]');
      const textElement = showcaseElement.querySelector('.discount-showcase__text');
      const badgeElement = showcaseElement.querySelector('[data-discount-badge]');
      const badgeTextElement = showcaseElement.querySelector('[data-discount-badge-text]');
      const isPDP = showcaseElement.classList.contains('discount-showcase--pdp');
      ...
      if (textElement) {
        const withCodeText = window.themeVariables?.strings?.discountWithCode || 'with your code';
        const textPrefixElement = textElement.querySelector('[data-discount-text-prefix]');
        const codeElement = textElement.querySelector('[data-discount-code]');
        ...
        if (textPrefixElement && codeElement) {
          ...
          if (isPDP && codeButton) {
            ...
            if (isCodeApplied) {
              ...
              if (appliedIcon) appliedIcon.style.display = 'inline-flex';
              codeButton.style.cursor = 'default';
              codeButton.setAttribute('aria-label', 'Discount code applied');
            } else {
              codeButton.classList.remove('is-applied');
              if (copyIcon) copyIcon.style.display = 'inline-flex';
              ...
              newButton.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                this.copyCodeToClipboard(activeCode);
              });
            }
          }
        } else {
          textElement.textContent = `${withCodeText} ${activeCode}`;
        }
      }

      if (badgeElement && showSecondBadge && badgeTextElement) {
        const extraSaveText = window.themeVariables?.strings?.discountExtraSave || 'Extra save {{ percentage }}%';
        badgeTextElement.textContent = extraSaveText.replace('{{ percentage }}', savingsPercentage).replace('{{percentage}}', savingsPercentage).replace('@percentage@', savingsPercentage);
        badgeElement.style.display = '';
      } else if (badgeElement) {
        badgeElement.style.display = 'none';
      }

      this.showShowcase(showcaseElement);
    }

    hideShowcase(element) {
      element.classList.remove('discount-showcase--loading', 'discount-showcase--visible');
      element.classList.add('discount-showcase--hidden');
    }

    showShowcase(element) {
      element.classList.remove('discount-showcase--loading', 'discount-showcase--hidden');
      element.classList.add('discount-showcase--visible');
    }
    ...
    extractPrefix(code) {
      ...
      if (underscoreIndex > 0) {
        return code.substring(0, underscoreIndex + 1);
      }
      return code;
    }

    async getDiscountPercentageForPrefix(prefix) {
      if (!this.discountPrefixes) {
        await this.loadDiscountPrefixes();
      }

      if (this.discountPrefixes && this.discountPrefixes[prefix]) {
        return this.discountPrefixes[prefix];
      }

      const prefixWithoutUnderscore = prefix.replace('_', '');
      if (this.discountPrefixes && this.discountPrefixes[prefixWithoutUnderscore]) {
        return this.discountPrefixes[prefixWithoutUnderscore];
      }
    ...
    formatMoney(amount) {
      if (window.themeVariables?.settings?.currencyCodeEnabled) {
        return new Intl.NumberFormat(window.themeVariables.localization.language || 'en', {
          style: 'currency',
          currency: window.themeVariables.settings.cartCurrency || 'EUR'
        }).format(amount / 100);
      } else {
        ...
        return format
          ...
          .replace(/\{\{amount_no_decimals\}\}/g, Math.round(amount / 100).toString())
          .replace(/\{\{amount_with_comma_separator\}\}/g, formattedAmount.replace('.', ','));
      }
    }
    ...
    copyCodeToClipboard(code) {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(code).then(() => {
          if (window.toastr) {
            window.toastr.success('Erfolgreich kopiert ✔️ Füge deinen Rabattcode im Checkout ein!');
          }
        }).catch((err) => {
  ...
  window.checkDiscountPrefixes = function() {
    if (window.discountShowcaseInstance) {
      console.log('==================================');
      console.log('Discount Showcase - Loaded Prefixes');
      console.log('==================================');
      console.log('Prefixes:', window.discountShowcaseInstance.discountPrefixes);
      console.log('==================================');
    } else {
      console.error('❌ DiscountShowcase instance not found');
    }
  };
